...
public class Stop implements Writeable {
     ...
     *
     * @param stopString The string to be decoded into a Stop.
    ...
    public static Stop decode(String stopString)
        ...
        if ((stopString == null) || (stopString.startsWith(":"))) {
            throw new TransportFormatException();
        }

        String[] stopInfo = stopString.split(":", 3);

        // ensure that the string is in the correct format,
        // (if not throw an exception)
        if ((stopInfo.length != 3) || (stopInfo[2].contains(":"))) {
            throw new TransportFormatException();
        } else try {
            return new Stop(stopInfo[0], Integer.parseInt(stopInfo[1].trim()),
                    Integer.parseInt(stopInfo[2].trim()));
